...
modules/fc-emulator/
  ...
  js/
    ...
    storage.js            # [DONE] StorageManager（localStorage 封装，base64 存储）
    rom-loader.js         # [DONE] ROMLoader（File API + ArrayBuffer + JSZip）
...
所有模块文件使用 ES Module (`export class` / `import`)，通过 `<script type="module">` 加载。
...
- ROM 数据以 **base64 格式**存储（`Uint8Array` ↔ base64 互转）
- 管理游戏状态、核心偏好的存取
- 处理 QuotaExceededError

**4. ROMLoader — ROM 加载**

- 封装文件选择（File System Access API + fallback input）
- 使用 `File.arrayBuffer()` + `Uint8Array`（避免 binary string 的 UTF-16 问题）
- 封装 ZIP 解压（JSZip）+ `.nes` 自动提取
...
- 支持 turbo 模式（快速跳过无声帧）
- `loadROM` 统一接受 `Uint8Array` / `ArrayBuffer` / binary string

**7. RetroArchCore — Nostalgist.js 适配器**

- 封装 `Nostalgist.launch()`, `pressDown/Up()`, `saveState/loadState()`, `exit()`
- 动态 `import()` Nostalgist.js 和核心文件
- `destroy(removeCanvas)` 接受参数区分停止游戏和返回启动器
...
| ROM binary string 格式问题 | 统一用 `Uint8Array` + base64 存储 |

## 实现注意事项

- `jsnes.NES` 和 `JSZip` 是全局变量，在 ES Module 中通过 `window.jsnes` / `window.JSZip` 访问
- `nostalgist.js` 保持动态 `import()` 加载方式不变
- CSS 保持内联在 index.html 的 `<style>` 中，不单独拆分
- DOM 元素引用在 UI 类初始化时一次性缓存
- `EmulatorCore.destroy(removeCanvas)` 接受参数区分游戏停止和返回启动器场景
- ROM 数据使用 `Uint8Array`，localStorage 中存为 base64 格式
