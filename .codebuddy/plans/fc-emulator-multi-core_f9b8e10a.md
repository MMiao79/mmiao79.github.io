---
name: fc-emulator-multi-core
overview: 为 FC 红白机模拟器添加多核心支持。保留 jsnes 作为默认核心，通过 Nostalgist.js 集成 fceumm、nestopia、quicknes 三个 RetroArch 核心作为备用。实现自动+手动核心选择机制，所有核心文件本地托管提供离线能力，保留全部现有 UI 功能。
todos:
  - id: download-core-files
    content: 下载 Nostalgist.js 库和 3 个 RetroArch 核心（fceumm/nestopia/quicknes）的 JS+WASM 文件到 lib/ 目录，确保所有文件可离线访问
    status: completed
  - id: implement-core-abstraction
    content: 实现核心抽象层（JsnesCore + RetroArchCore 适配器类、CoreManager 管理器）和双画布容器系统，重写 index.html 的核心引擎加载逻辑
    status: completed
    dependencies:
      - download-core-files
  - id: add-core-selection-ui
    content: 添加核心选择 UI：启动器界面核心选择控件、游戏界面核心指示器+切换按钮、自动检测失败弹窗、localStorage 核心偏好记忆
    status: completed
    dependencies:
      - implement-core-abstraction
  - id: adapt-controls-multicore
    content: 适配所有控制功能（键盘映射、虚拟摇杆、触摸按钮、连发键、存档恢复）统一通过 activeCore 适配层调用，支持核心切换
    status: completed
    dependencies:
      - implement-core-abstraction
  - id: polish-and-test
    content: 完善错误处理、加载状态指示、WASM 支持检测、边界情况处理，验证所有核心可正常加载和运行
    status: completed
    dependencies:
      - add-core-selection-ui
      - adapt-controls-multicore
---
